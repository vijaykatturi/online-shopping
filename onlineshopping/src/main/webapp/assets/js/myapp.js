$(function(){
	...
	if($table.length){
		...
		var jsonUrl = '';
		if(window.categoryId == ''){ //i.e,user is asking for all the products
	...
	//dismissing the alert mesaage of product submission to db after 3 seconds
	var $alert = $('.alert');
	if($alert.length){
		setTimeout(function(){
			$alert.fadeOut('slow');			
		...
		$categoryForm.validate({
			...
			messages:{
				name:{
					...
					minlenth : 'The categoyr name should not be less than 2 characters!!'
				},
				description:{
...
//validation code for Login Form
	...
	$('button[name="refreshCart"]').click(function(){
		...
		//work only when the count has benn changed
